...
sock.connect(("127.0.0.1", 5000))  # .NET'in dinlediği adres/port
...
def run_plate_reader():
    ...
    while True:
        ...
        if key == ord('q'):
            ...
            return last_valid_plate   # <<< BURADA PLAKA GERİ DÖNÜYOR

    cap.release()
    cv2.destroyAllWindows()
    return last_valid_plate
...
while True:
    ...
    for cnt in cnts:
        ...
        if len(approx) == 4:
            x, y, w, h = cv2.boundingRect(approx)
            aspect_ratio = w / float(h)
            if aspect_ratio < 1.0 or aspect_ratio > 12.0:  # genişletildi
                continue
            if w < 80 or h < 20:
                continue
    ...
    if plate_detected and last_valid_plate:
        ...
        try:
            sock.sendall(last_valid_plate.encode() + b"\n")
        except:
            pass
...
plate = run_plate_reader()
print("Okunan plaka:", plate)
